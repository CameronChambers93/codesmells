...
public class NodeTransfer implements NodeVisitor {
    ...
    public void addSet(WriteVariableNode n) {
        String kind = "Null";
        Function f = n.getBlock().getFunction();
        Value v = c.getState().readRegister(n.getValueRegister());
        if (!(v.getAllObjectLabels().isEmpty())) {
            for (ObjectLabel objectLabel: v.getAllObjectLabels())
                kind = objectLabel.getKind().toString();
        }
        if (!v.isNotBool())
            kind = "Boolean";
        else if (!v.isNotStr())
            ...
            ; // Maybe put more here?
        if (kind != "Null") {
            detectReassignment(n, kind);
            f.variableValueTypes.put(n.getVariableName(), kind);
        }
    }

    public boolean detectReassignment(WriteVariableNode n, String kind) {
        Function f = n.getBlock().getFunction();
        if (!(f.variableValueTypes.containsKey(n.getVariableName())))
            return false;
        else
            if (f.variableValueTypes.get(n.getVariableName()) != kind) {
                if (f.reassignedVariableStrings.add(n.getVariableName()))
                    f.reassignedVariables.add(n);
            }
        return false;
    }
    ...
    public void visit(WritePropertyNode n) {
        ...
        c.getState().writeRegister(n.getBaseRegister(), Value.makeObject(objlabels)); // if null/undefined, an exception would have been thrown via toObjectLabels
        // get the property name value, separate the undefined/null/NaN components, coerce with ToStringd
        Value propertyval;
        if (n.isPropertyFixed()) {
            propertyval = Value.makeStr(n.getPropertyString());
        } else {
    ...
    public void visit(DeletePropertyNode n) {
        ...
        if (n.isVariable()) {
            ...
            Set<ObjectLabel> objlabels = Conversion.toObjectLabels(n, baseval, c);
            for (ObjectLabel objs: objlabels)
            if (objlabels.isEmpty() && !Options.get().isPropagateDeadFlow()) {
                c.getState().setToNone();
                return;
            }
